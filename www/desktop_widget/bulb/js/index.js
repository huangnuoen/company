...
define(['jquery','OomiCommand','util'],
    function($,OC,popupTools) {
        ...
        function getType(){
            var p = Oomi_command_function.create_new().resource_data();
            if(p['widgetType'] == 'widgetLedStrip'){
                return 'colorStrip';
            }else{
                return 'bulb';
            }
            ...
            function getDeviceData() {
                cordova.exec(function (data) {
                    ...
                    popupTools.updateUI(data, 'color', function plugUpdate(result) {
                        if(result == null){
                            cordova.exec(null,null,"FTP2PApi",'deviceControl',[nodeid,"rgbbulb01",'warmWhite',"color"]);
                        }else{
                            Color(result);
                        }
                    });
