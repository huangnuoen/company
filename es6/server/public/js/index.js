	...
	var Lottery = function (_mix) {
		...
		_createClass(Lottery, [{
			...
			value: function initEvent() {
				var self = this;
				//玩法绑定
				(0, _jquery2.default)('#plays').on('click', 'li', self.changePlayNav.bind(self)); //把this指向调用initEvent的当前对象
				//号码绑定
	...
	var Base = function () {
		...
		_createClass(Base, [{
			...
			value: function toggleCodeActive(e) {
				var self = this;
				var $cur = (0, _jquery2.default)(e.currentTarget); //触发事件的元素
			...
			value: function assistHandle(e) {
				...
				if (index === 1) {
					(0, _jquery2.default)('.boll-list .btn-boll').each(function (i, item) {
				...
				(function f(arr, size, result) {
					...
					if (size === arrLen) {
						...
						for (var i = 0; i < arrLen; i++) {
							var newResult = [].concat(result); //生成一个新数组
							newResult.push(arr[i]); //添加参加运算的数组元素
							if (size === 1) {
								//基数为1时，说明已经到最后一个元素，添加新结果，不需再递归
